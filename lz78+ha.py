...
def files():
    ...
    if number == 1: return "enwik7", "LZ78+HA_enwik7_encoded.txt", "LZ78+HA_enwik7_decoded.txt"
    if number == 2: return "чехов.txt", "LZ78+HA_чехов_encoded.txt", "LZ78+HA_чехов_decoded.txt"
    if number == 3: return "SafeMain.exe", "lz78+HA_exe_encoded.txt", "lz78+HA_exe_decoded.txt"
    if number == 4: return "фото_чб.bmp", "lz78+HA_ФОТО_ЧБ_encoded.txt", "lz78+HA_ФОТО_ЧБ_decoded.txt"
    if number == 5: return "piter.bmp", "lz78+HA_ФОТО_СЕРОЕ_encoded.txt", "lz78+HA_ФОТО_СЕРОЕ_decoded.txt"
    if number == 6: return "panda.bmp", "LZ78+HA_ФОТО_ЦВЕТ_encoded.txt", "LZ78+HA_ФОТО_ЦВЕТ_decoded.txt"
    if number == 7: return "english.txt", "LZ78+HA_english_encoded.txt", "LZ78+HA_english_decoded.txt"
...
def compress_and_decompress(input_file_path, compressed_file_path, decompressed_file_path):
    ...
    en_data1 = lz78_encode(original_data)
